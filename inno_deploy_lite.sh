#!/bin/bash
# =============================================================================
# Inno-Pilot LITE deployment script (no git pull)
# Resides at ~/inno_deploy_lite.sh on the Pi (OUTSIDE the repo).
#
# Usage:
#   bash ~/inno_deploy_lite.sh
#
# Same as inno_deploy.sh but skips the git pull step.
# Use this to re-deploy whatever is already checked out in ~/inno-pilot
# without fetching from GitHub (e.g., after manual edits or when offline).
#
# What this script does:
#   1. Pre-compile Nano firmware (while services are still up — no port needed)
#   2. Stop all inno-pilot + pypilot services
#   3. Deploy glue (bridge, web-remote, systemd units, OTA binary)
#   4. Flash the Nano via arduino-cli
#   5. Wait for the Nano to boot after flash
...
# ---------------------------------------------------------------------------
info "Step 1 — Pre-compile Nano firmware (services still running)"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
info "Step 2 — Stop services (consumers first, providers last)"
# ---------------------------------------------------------------------------
stop_svc() {
    local svc="$1"
...
# ---------------------------------------------------------------------------
info "Step 3 — Deploy glue (bridge, web-remote, systemd units, OTA binary)"
# ---------------------------------------------------------------------------
bash "$GLUE_DEPLOY"
log "Glue deploy complete."

# ---------------------------------------------------------------------------
info "Step 4 — Flash Nano firmware"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
info "Step 5 — Waiting ${NANO_BOOT_WAIT_S}s for Nano to complete setup() after flash"
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
info "Step 6 — Start services (providers first, consumers last)"
# ---------------------------------------------------------------------------
start_svc() {
    local svc="$1"
...
# ---------------------------------------------------------------------------
info "Step 7 — Service status summary"
# ---------------------------------------------------------------------------
